...
class ReverbDataset(agentos.Dataset):
    ...
    def ready_to_initialize(cls, shared_data):
        return ('environment_spec' in shared_data and
                'network' in shared_data)
    ...
    def add(self, prev_obs, action, curr_obs, reward, done, info):
        if action is None:  # No action -> first step
            ...
            if done:
                timestep = TimeStep(
                    StepType.LAST, reward, self.shared_data['discount'], curr_obs
                )
            else:
                timestep = TimeStep(
                    StepType.MID, reward, self.shared_data['discount'], curr_obs
                )
